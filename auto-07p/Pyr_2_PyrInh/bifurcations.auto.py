...
# To get the lc, we first take eps=0 and large p, and then increase eps to find a Hopf. We stop at our desired eps=10
...
fp=run(ic,IPS=1,NMX=10000,ISW=1,ICP=[2,1], UZSTOP={'eps' : 30.0})   # 1-param bifurcations for the steady state

# Limit cycle from a Hopf (along eps, fixed p=400)
hb=fp('HB1')                                                        # Hopf bifurcation
lc0=run(hb,IPS=2,ISP=2,ICP=[2,11,1,3,4],NMX=10000,ISW=1)             # Continuation of limit-cycle increasing eps until eps=10

# Limit cycle from user defined point (along p, fixed eps=10.0)
lc_ic=lc0('UZ1')
...
# Steady states along p with fixed eps=10.0
init=run('jrnmm',IPS=-2,NMX=100000,PAR={'p': -100.0, 'eps' : 10.0})   
ic=init(201)                                                        
...
loop=lc1(236)
loop=run(loop,IPS=2,ISP=2,ICP=[1,11,2,3,4],NMX=10000,ISW=1,UZSTOP={'p':120},DS="-");

for fl_p in range(121,600,1):
    ...
    loop=run(loop('UZ1'),IPS=2,ISP=2,ICP=[1,11,2,3,4],NMX=10000,ISW=1,UZSTOP={'p':fl_p});
