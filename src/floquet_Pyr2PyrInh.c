...
void velocity_fields(gsl_vector *x, gsl_vector *k, double p, double eps, double lam)
{
	double y0,y1,y2,y3,y4,y5,x1,x2;
	int n=SYS_N;
       	...
       	VECTOR(k,1)=y4;
       	VECTOR(k,2)=y5;
       	VECTOR(k,3)=PAR_a*( PAR_A*Sigm(y1-y2 +x1)-2*y3-PAR_a*y0);
...
void jacobian(gsl_matrix *j, gsl_vector *x, double eps, double lambda )
{
        ...
        double y1=VECTOR(x,1);
	double y2=VECTOR(x,2);
	double x1=VECTOR(x,6);
        ...
        MATRIX_SET(j,4,1, -PAR_a*PAR_a);
        MATRIX_SET(j,5,2, -PAR_b*PAR_b);


        MATRIX_SET(j,3,1, PAR_a*PAR_A*dSigm(y1-y2+x1) );
        MATRIX_SET(j,3,2, -PAR_a*PAR_A*dSigm(y1-y2+x1) );
        MATRIX_SET(j,3,6, PAR_a*PAR_A*dSigm(y1-y2+x1) );

        MATRIX_SET(j,4,0, PAR_a*PAR_A*PAR_C1*PAR_C2*dSigm(PAR_C1*y0) );
        MATRIX_SET(j,5,0, PAR_b*PAR_B*PAR_C3*PAR_C4*dSigm(PAR_C3*y0+x1) );
